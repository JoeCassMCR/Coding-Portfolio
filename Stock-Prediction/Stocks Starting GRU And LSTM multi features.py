...
scaled_data  = np.hstack([scaled_close, scaled_other])
...
def make_model(layer):
    m = Sequential([
        layer(128, return_sequences=True, input_shape=(X_tr.shape[1], X_tr.shape[2])),
        Dropout(0.2),
        layer(64),
        Dropout(0.2),
        Dense(1)
...
plt.plot(h_gru.history['val_loss'], label='GRU Val Loss')
...
# 11) Plot Predictions vs Actual (first 100 timesteps)
plt.figure(figsize=(10,5))
plt.plot(y_actual[:100], label='Actual')
plt.plot(y_gru   [:100], label='GRU Pred')
plt.plot(y_lstm  [:100], label='LSTM Pred')
plt.title('First 100 Days: Actual vs GRU vs LSTM')
